...
export function FloatingCreateButton({ className, mobileFab }: Props) {
  if (mobileFab) {
    return (
      <Link
        ...
        className={cn(
          "md:hidden fixed bottom-20 right-6 z-40 w-14 h-14 rounded-full bg-[#111827] text-white shadow-xl flex items-center justify-center active:scale-[0.95] transition-all",
          className
        )}
  ...
  return (
    <div className={cn("absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex justify-center", className)}>
      <Link
        href="/assignments/new"
